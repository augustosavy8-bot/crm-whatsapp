...
// Aviso al staff del gimnasio cuando un alumno se anota, para que confirme el
// turno. Web Push a las suscripciones del tenant; best-effort (nunca rompe la
// reserva). No-op si no hay VAPID configurado.
...
export async function notificarReservaGym(args: {
  ...
  try {
    ...
    await sendPushToTenant(args.tenantId, {
      title: "Nuevo turno para confirmar",
      body: `${args.nombre} se anotó · ${cuando}. Tocá para confirmarlo.`,
      url: "/gym",
      tag: "gym-reserva",
    });
  } catch {
